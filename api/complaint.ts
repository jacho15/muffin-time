...
// Emails feedback from a signed-in user to the app owner via Resend.
...
export async function POST(req: Request): Promise<Response> {
  const { SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, RESEND_API_KEY, COMPLAINT_TO_EMAIL, COMPLAINT_FROM_EMAIL } =
    process.env
  if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY || !RESEND_API_KEY || !COMPLAINT_TO_EMAIL || !COMPLAINT_FROM_EMAIL) {
  ...
  const subject = body.subject?.trim().slice(0, MAX_SUBJECT_LENGTH) || 'No subject'
  ...
  try {
    ...
    return Response.json({ error: 'Email service unreachable, try again later' }, { status: 502 })
  }

  if (res.status === 429) {
    return Response.json({ error: 'Too many emails right now, try again in a minute' }, { status: 429 })
  }
  if (!res.ok) {
