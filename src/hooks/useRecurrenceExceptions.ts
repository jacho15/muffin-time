...
export function useRecurrenceExceptions() {
  ...
  const createException = useCallback(
    async (exc: RecurrenceExceptionInsert) => {
      ...
      if (data) {
        setExceptions(prev => {
          const idx = prev.findIndex(
            e =>
              e.parent_type === data.parent_type &&
  ...
  const deleteExceptionsForParent = useCallback(
    async (parentType: string, parentId: string) => {
      await supabase.from('recurrence_exceptions').delete().eq('parent_type', parentType).eq('parent_id', parentId)
      setExceptions(prev => prev.filter(e => !(e.parent_type === parentType && e.parent_id === parentId)))
    },
    [setExceptions],
  )
