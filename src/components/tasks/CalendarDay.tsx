...
function CalendarDayComponent({
  ...
  return (
    <div
      ref={setNodeRef}
      className={`bg-void/50 p-1.5 min-h-[80px] cursor-pointer transition-colors [content-visibility:auto] [contain-intrinsic-size:auto_80px] ${
      ...
      >
        <div className="flex flex-col gap-0.5">
          {dayOccurrences.slice(0, 3).map(occ => {
            ...
            return (
              <SortableTaskItem
                ...
                className={`text-[11px] px-1 py-0.5 rounded transition-colors cursor-pointer ${
                  completed ? 'text-star-white/50' : 'text-white'
                } ${selectedItemId === item.id ? 'ring-1 ring-gold' : ''}`}
                style={{
                  backgroundColor: completed ? 'rgba(255,255,255,0.03)' : getCourseItemColor(assignment.course) + '20',
                }}
          ...
          {dayOccurrences.length > 3 && (
            <div className="text-[10px] text-star-white/60 px-1">+{dayOccurrences.length - 3} more</div>
          )}
        </div>
      </SortableContext>
    </div>
