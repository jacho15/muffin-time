...
interface TaskModalProps {
  ...
  createException: (exception: RecurrenceExceptionInsert) => Promise<RecurrenceException>
...
const TaskDueDateRow = memo(function TaskDueDateRow({
  ...
  return <MemoDatePicker value={value} onChange={onChange} />
})
...
const TaskRepeatsRow = memo(function TaskRepeatsRow({
  ...
  return (
    <div className="relative">
      ...
      >
        <span className="truncate">{RECURRENCE_OPTIONS.find(opt => opt.value === value)?.label}</span>
        <ChevronDown size={14} className={`text-star-white/50 transition-transform ${isOpen ? 'rotate-180' : ''}`} />
      </button>
      <div
        className={`absolute top-full left-0 mt-1 w-full min-w-[140px] rounded-lg border border-glass-border z-[60] overflow-hidden cosmic-glow shadow-2xl transition-all duration-100 origin-top ${
...
export default function TaskModal({
    ...
    if (initialItem) {
      return {
        ...
        dueDate: initialOccurrence
          ? initialOccurrence.occurrenceDate
          : mode === 'todos'
        ...
        status:
          ((initialItem as Record<string, unknown>).status as string) ||
          (initialItem.completed ? 'Completed' : 'Not Started'),
  ...
  const filteredTypeOptions = useMemo(() => typeOptions.filter(t => t !== 'Todo' && t !== 'Assignment'), [typeOptions])

  const handleTitleChange = useCallback((value: string) => setForm(f => ({ ...f, title: value })), [])
  const handleTypeChange = useCallback((value: string) => setForm(f => ({ ...f, type: value })), [])
  const handleCourseChange = useCallback((value: string) => setForm(f => ({ ...f, course: value })), [])
  const handleDueDateChange = useCallback((value: string) => setForm(f => ({ ...f, dueDate: value })), [])
  ...
  const handleStatusChange = useCallback((value: string) => setForm(f => ({ ...f, status: value })), [])
  useEffect(() => {
      ...
      >
        <div className="px-6 pt-6 pb-2 flex items-start justify-between">
          <input
            ...
            autoFocus
          />
          <button onClick={onClose} className="p-1 rounded hover:bg-glass-hover text-star-white/50 ml-2 shrink-0">
            <X size={18} />
          </button>
        </div>
        ...
        <div className="px-6 pb-4">
          <div className="grid gap-y-3" style={{ gridTemplateColumns: '120px 1fr' }}>
            ...
            <TaskDueDateRow value={form.dueDate} onChange={handleDueDateChange} />
            ...
            {form.recurrence !== 'once' && (
              <>
                ...
                <TaskRepeatUntilRow value={form.recurrenceUntil} onChange={handleRecurrenceUntilChange} />
              </>
            )}
