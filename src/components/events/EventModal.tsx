...
const CalendarSelectRow = memo(function CalendarSelectRow({
  ...
  return (
    <div className="relative">
        ...
        <span className="truncate">
          {selectedCalendar ? (
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full" style={{ backgroundColor: selectedCalendar.color }} />
      ...
      <div
        className={`absolute top-full left-0 mt-1 w-full rounded-lg border border-glass-border z-[60] overflow-hidden cosmic-glow transition-all duration-100 origin-top ${
...
export default function EventModal({
  ...
  // Reset the form whenever the modal opens or is re-seeded (adjusting state
  // during render, instead of in an effect, avoids an extra render pass).
  const [seed, setSeed] = useState({ isOpen, defaultFormState })
  if (seed.isOpen !== isOpen || seed.defaultFormState !== defaultFormState) {
    setSeed({ isOpen, defaultFormState })
    if (isOpen) {
  ...
  const isRecurring = (event: CalendarEvent | null) => event?.recurrence && event.recurrence !== 'once'

  const handleTitleChange = useCallback((value: string) => setForm(f => ({ ...f, title: value })), [])
  const handleDescriptionChange = useCallback((value: string) => setForm(f => ({ ...f, description: value })), [])
  const handleStartTimeChange = useCallback((value: string) => setForm(f => ({ ...f, start_time: value })), [])
  const handleEndTimeChange = useCallback((value: string) => setForm(f => ({ ...f, end_time: value })), [])
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-medium text-star-white">{editingEvent ? 'Edit Event' : 'New Event'}</h3>
              ...
              <div className="flex flex-col gap-3">
                ...
                {form.recurrence !== 'once' && (
                  <div>
                    ...
                    <MemoDatePicker value={form.recurrence_until} onChange={handleRecurrenceUntilChange} />
                  </div>
                )}
